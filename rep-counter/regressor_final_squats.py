...
current_leg: str = "right"  # Currently tracked leg (left or right)
...
class SquatRegressor(nn.Module):
    """
    Neural network model for squat position regression.
    
    Takes normalized keypoint coordinates as input and outputs a continuous
    value between 0-1 representing squat depth/position.
    """
    
    def __init__(self, input_feature_count: int = 4) -> None:
        """
        Initialize the squat regression model.
...
def normalize_keypoint_coordinates(raw_keypoints: list[float]) -> list[float]:
    """
    Normalize keypoint coordinates relative to knee position.
    
    This reduces the impact of camera distance and person size by making
    all measurements relative to the knee joint.
    
    Args:
        raw_keypoints: List of 6 values [ankle_x, ankle_y, knee_x, knee_y, hip_x, hip_y]
        
    Returns:
        List of 4 normalized values relative to knee position
    """
    # Extract knee coordinates as reference point
    knee_x_coordinate, knee_y_coordinate = raw_keypoints[2], raw_keypoints[3]
    
    return [
        raw_keypoints[0] - knee_x_coordinate,  # Ankle x relative to knee
        raw_keypoints[1] - knee_y_coordinate,  # Ankle y relative to knee
        raw_keypoints[4] - knee_x_coordinate,  # Hip x relative to knee
        raw_keypoints[5] - knee_y_coordinate   # Hip y relative to knee
    ]


def draw_selected_leg_landmarks(frame: cv2.Mat, pose_results: mp.solutions.pose.Pose) -> None:
    """
    Draw pose landmarks and connections for the currently selected leg.
    
    Only draws the leg with better visibility to reduce visual clutter.
    ...
    # MediaPipe landmark indices for right and left leg joints
    RIGHT_LEG_INDICES = [24, 26, 28]  # [hip, knee, ankle]
    LEFT_LEG_INDICES = [23, 25, 27]   # [hip, knee, ankle]
    
    selected_leg_indices = RIGHT_LEG_INDICES if current_leg == "right" else LEFT_LEG_INDICES

    # Check if all landmarks are visible enough to draw
    if not all(landmarks[i].visibility > 0.5 for i in selected_leg_indices):
        return

    # Draw keypoints as circles
    for landmark_index in selected_leg_indices:
    ...
    leg_connections = [(selected_leg_indices[0], selected_leg_indices[1]), 
                       (selected_leg_indices[1], selected_leg_indices[2])]
    
    for start_joint_index, end_joint_index in leg_connections:
...
def extract_and_select_best_leg_keypoints(pose_results: mp.solutions.pose.Pose) -> list[float] | None:
    """
    Extract keypoints from the leg with better visibility.
    
    Automatically switches between left and right leg based on visibility scores
    to ensure consistent tracking even when one leg is occluded.
    
    Args:
        pose_results: MediaPipe pose detection results
        
    Returns:
        List of normalized keypoint coordinates or None if no valid leg found
    """
    global current_leg
    ...
    # Calculate visibility scores for both legs
    right_leg_visibility = [landmarks[i].visibility for i in RIGHT_LEG_INDICES]
    left_leg_visibility = [landmarks[i].visibility for i in LEFT_LEG_INDICES]

    # Check which legs have sufficient visibility
    is_right_leg_valid = all(visibility > 0.5 for visibility in right_leg_visibility)
    is_left_leg_valid = all(visibility > 0.5 for visibility in left_leg_visibility)

    # Return None if neither leg is sufficiently visible
    if not is_right_leg_valid and not is_left_leg_valid:
        return None

    # Calculate total visibility scores for leg switching logic
    right_leg_total_visibility = sum(right_leg_visibility)
    left_leg_total_visibility = sum(left_leg_visibility)

    # Switch to better leg if visibility difference is significant
    visibility_threshold = 0.05
    if current_leg == "right" and is_left_leg_valid and (left_leg_total_visibility - right_leg_total_visibility) > visibility_threshold:
        current_leg = "left"
    elif current_leg == "left" and is_right_leg_valid and (right_leg_total_visibility - left_leg_total_visibility) > visibility_threshold:
        current_leg = "right"

    # Extract coordinates for selected leg
    selected_leg_indices = RIGHT_LEG_INDICES if current_leg == "right" else LEFT_LEG_INDICES
    raw_keypoints = [
        landmarks[selected_leg_indices[2]].x, landmarks[selected_leg_indices[2]].y,  # Ankle
        landmarks[selected_leg_indices[1]].x, landmarks[selected_leg_indices[1]].y,  # Knee
        landmarks[selected_leg_indices[0]].x, landmarks[selected_leg_indices[0]].y   # Hip
    ]
    
    return normalize_keypoint_coordinates(raw_keypoints)


def classify_squat_state_using_waves(model: SquatRegressor, normalized_keypoints: list[float]) -> float:
    """
    Classify current squat state using wave-based detection method.
    
    Processes keypoints through the neural network and updates the state sequence
    for wave pattern detection and rep counting.
    
    Args:
        model: Trained squat regression model
        normalized_keypoints: List of normalized keypoint coordinates
        
    Returns:
        Continuous value between 0-1 representing squat depth
    """
    model.eval()
    with torch.no_grad():
        ...
        squat_depth_value = model_output.item()
        
        # Update state sequence for wave detection
        update_state_sequence_for_wave_detection(squat_depth_value)
...
def convert_continuous_value_to_discrete_state(continuous_value: float) -> int:
    ...
    if 0.0 <= continuous_value < 0.2:
        return 1  # Standing/highest position
    elif 0.2 <= continuous_value < 0.4:
        return 2  # Quarter squat
    elif 0.4 <= continuous_value < 0.6:
        return 3  # Half squat
    elif 0.6 <= continuous_value < 0.8:
        return 4  # Three-quarter squat
    elif 0.8 <= continuous_value <= 1.0:
        return 5  # Deep squat/lowest position
    else:
        return 1  # Default to standing for invalid values


def update_state_sequence_for_wave_detection(squat_depth_value: float) -> None:
    """
    ...
    Args:
        squat_depth_value: Current continuous squat depth value
    """
    global state_sequence_list, current_wave_state
    
    new_discrete_state = convert_continuous_value_to_discrete_state(squat_depth_value)
...
def detect_repetitions_using_wave_analysis() -> list:
    """
    Detect complete squat repetitions using wave pattern analysis.
    
    Uses signal processing techniques to identify complete squat cycles
...
def create_live_plot_visualization() -> None:
    """
    Create and manage live plotting of squat depth values.
    
    Runs in a separate thread to provide real-time visualization of
    squat progression with state boundaries and color coding.
    """
    ...
    axis.set_title('Live Squat Position Value')
    axis.grid(True)
    
    # Add horizontal lines showing state boundaries
    ...
    # Control update frequency to balance responsiveness and performance
        ...
        try:
            ...
            if current_time - last_update_time >= plot_update_interval:
                if len(plot_values) > 0:
                    # Prepare data for last 200 points
                    ...
                    # Color line based on current squat state
...
def run_squat_analysis_inference(video_source: int | str = 0, model_file_path: str = "squat_regressor_sigmoid_both_leg.pth") -> None:
    """
    Main inference function for wave-based squat analysis.
    ...
    """
    global current_leg, wave_repetition_counter, video_frames_per_second, target_frame_duration, last_frame_timestamp
    
    # Setup device and model
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    squat_model = SquatRegressor()
    squat_model.load_state_dict(torch.load(model_file_path, map_location=device))
    ...
    while video_capture.isOpened():
        ...
        if pose_detection_results.pose_landmarks:
            draw_selected_leg_landmarks(current_frame, pose_detection_results)
            extracted_keypoints = extract_and_select_best_leg_keypoints(pose_detection_results)
            
            if extracted_keypoints is not None:
                squat_depth_value = classify_squat_state_using_waves(squat_model, extracted_keypoints)
                if squat_depth_value is not None:
                    plot_values.append(squat_depth_value)
        ...
        if len(plot_values) > 0:
            current_squat_value = plot_values[-1]
            current_discrete_state = convert_continuous_value_to_discrete_state(current_squat_value)
            cv2.putText(current_frame, f"State: {current_discrete_state} ({current_squat_value:.3f})", (20, 80), 
        ...
        cv2.imshow('Squat Analysis - Wave Detection', current_frame)
...
if __name__ == "__main__":
    ...
    # run_squat_analysis_inference(0)  # Use live camera
    run_squat_analysis_inference("x_cut.mp4")  # Alternative: use video file
