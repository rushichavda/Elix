...
class RepCounter:
    """Universal rep counter for various exercises with improved logic"""
    ...
    def get_exercise_angles(self, keypoints: np.ndarray) -> Dict[str, tuple]:
        ...
        try:
            ...
            # ... rest of the exercise types remain the same ...
    ...
    def plot_angle_trace(self, save_path: Optional[str] = None):
        ...
        for rep_frame in self.rep_timestamps:
            ax1.axvline(x=rep_frame, color='g', linestyle='--', alpha=0.7, linewidth=2, label='Rep Completed' if rep_frame == self.rep_timestamps[0] else "")

        # Mark rep phases
        for i, phase in enumerate(self.rep_phases):
            ax1.axvspan(phase['start_frame'], phase['end_frame'],
                        alpha=0.2, color='green', label='Rep Phase' if i == 0 else "")
        ...
        ax1.set_title(f'{self.exercise_type.title()} - Angle Trace (Reps: {self.rep_count})')
        ax1.legend()
